...
function loded(opt) {
  ...
  let cartReflect = JSON.parse(window.localStorage.getItem("cartItems"));
  cartCount = cartReflect.length;
  let category = gourment[opt];
  if (cartReflect.length > 0) {
    cartReflect.forEach((element) => {
      const itemIndex = category.findIndex((items) => items.title === element.title);
      if (itemIndex > -1) {
  ...
  document.querySelectorAll(".plus_minus_btn").forEach((item) => {
    item.addEventListener("click", (event) => {
      ...
      if (targetedButton.charAt(0) == "+") {
        var itemC =
          document.getElementById(targetedButton).previousElementSibling;
        ...
        gourment[opt][targetedButton.charAt(1)].count =
          gourment[opt][targetedButton.charAt(1)].count + 1;
        if (
          selecedItems.some(
            (elem) =>
              elem.title === gourment[opt][targetedButton.charAt(1)].title
          )
        ) {
        ...
        if (itemCount == 0) {
          document.getElementById(targetedButton).parentElement.style.display =
            "none";
  ...
  document.querySelectorAll(".Btn_Cart").forEach((item) => {
    ...
    if (gourment[opt][item.id.charAt(4)].count > 0) {
      ...
      var itemC = document.getElementById(item.id).nextElementSibling
        .children[1];
    ...
    item.addEventListener("click", (event) => {
      ...
      var itemC =
        document.getElementById(targetedButton).nextElementSibling.children[1];
      itemCount = itemC.innerHTML;
      itemCount++;
      itemC.innerHTML = gourment[opt][targetedButton.charAt(4)].count + 1;
      document.getElementById("checkoutCart").innerHTML = ++cartCount;
      gourment[opt][targetedButton.charAt(4)].count =
        gourment[opt][targetedButton.charAt(4)].count + 1;
      if (
        selecedItems.some(
          (elem) => elem.title === gourment[opt][targetedButton.charAt(4)].title
        )
      ) {
        selecedItems = selecedItems.filter(function (obj) {
